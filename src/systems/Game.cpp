...
// Entity and Item headers are still pulled in for the strong types referenced
// elsewhere in this file (Player, EnemyAttackInfo, AbilityKind…); keeping them
// here means the unique_ptr destructors generated by GameState see complete
// types as required by C++17.
...
// Reset the run state for a fresh game. Delegates to GameState::reset() and
// EventLog::clear() so the existing state_ and log_ members are reused in
// place — no destruction or reallocation. This is the standard "rebuild via
// member functions" pattern.
void Game::resetRun(unsigned int seed)
{
    // Restore every counter, the dungeon map, the player, and the RNG seed.
    state_.reset(config_, seed);

    // Empty the linked-list of recent events from the previous run.
    log_.clear();

    // Generate the first wave's map and enemy roster.
    const int spawnCount = BASE_ENEMY_SPAWN_COUNT;
    waves_.startWave(state_, config_, spawnCount);

    // Hand the player its starting Dash / Nova / Shield / Blink kit (R21).
    grantStartingAbilities();

    // Match the streamed background track to the current wave. Wave 1 is a
    // normal wave so this picks the action theme.
    syncBossMusicForCurrentWave();

    // A run now exists in memory, so the main menu can offer "Resume Game"
    // if the player later leaves with Q.
    runInProgress_ = true;

    // Auto-save the freshly started run so Load Game can restore it later
    // even if the player never presses Ctrl+S.
